...
# ---- Check if already inside kernel_oneplus_sm8350 ----
[ "$(basename "$CURRENT_DIR")" = "kernel_oneplus_sm8350" ] && echo -e "${GREEN}Already inside kernel_oneplus_sm8350, skipping clone.${NC}" || {
	# Check if the directory does not exist, then remove and clone it
	[ ! -d "kernel_oneplus_sm8350" ] && {
		echo -e "${GREEN}Cloning and removing kernel_oneplus_sm8350 directory...${NC}"
		sudo rm -fr kernel_oneplus_sm8350 && git clone --depth=1 https://github.com/Madara273/kernel_oneplus_sm8350 -b r14
	}

	# Enter the directory if not already inside
	cd kernel_oneplus_sm8350
	echo -e "${GREEN}Entering kernel_oneplus_sm8350 directory...${NC}"
}
...
KBUILD_HOST="${KBUILD_HOST:-Kali_GNU/Linux-2025.2}"	# If user doesn't enter a value, use "Kali_GNU/Linux-2025.2"
